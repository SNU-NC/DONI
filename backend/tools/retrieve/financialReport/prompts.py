...
EXTRACT_PROMPT = PromptTemplate(
    template="""당신은 금융 전문가입니다. 약간의 단어 차이에도 민감하게 반응해야합니다. 다음 금융 문서와 테이블 정보에서 사용자의 질문과 관련된 정보를 찾아 형식에 맞게 추출하세요.
                ...
                3. 사용자가 원하는 단어와 완전히 일치하지 않으면 유의하라는 표시를 해주세요
                4. 관련이 없다면 빈 객체를 반환하세요
                4. 연도나 기수 정보가 있는 경우:
...
# 2. 계정명 확인: df['계정명'].unique()를 사용하여 데이터프레임에서 사용 가능한 모든 계정명을 확인합니다.
# 3. 계정명 선택: 
...
#    - 정보의 한계가 있다면 해당 데이터를 조심해서 쓰라는 말을 해주세요
...
COMBINED_FINANCIAL_REPORT_PROMPT = PromptTemplate(
    template="""당신은 사업보고서와 재무제표 도구의 결과를 그대로 출력하는 도구입니다. (사용자가 요청한 연도가 아니여도 최대한 일치하는 정보를 가져와야함)
    사용자의 요청 : {query}
...
2. 정보를 제공하는 입장에서 사용자가 바라는 모든 정보가 아주 명확하고 정확히 담겨있다면 그대로 답변을 제공하세요
3. 정보가 일부 손실되었거나 **사용자의 요청과 비슷하지만 약간이라도 다른 정보가** 나온다면 financial_report_search 도구를 호출하여 정확한 정보를 검색하세요
...
답변 출력 과정:
1. 각 도구['output'] 확인
2. 주어진 데이터에 존재하는 모든 연도 나열
3. 각 연도별 찾은 데이터들 나열
- 숫자 데이터가 괄호로 감싸져있으면, (3415) 이렇게 되어있으면 -3415원이라고 해주세요 해당 값은 손실입니다
4. 도구들에서 나온 결과를 **있는 그대로 출력**하는게 중요합니다 
5. 비슷한 정보가 있다면 최대한 사용자의 요청과 일치하는 값만 출력하세요 (다음 에이전트 헷갈리지 않게 최소한의 필터링)
- 수재보험료, 보험료수익 ,원수보험료, 영업수익, 영업손익과 같이 비슷한 정보가 있다면 최대한 사용자의 요청과 **좡확히 일치하는** 값만 출력하세요 연도별로 구분하여 출력 (해당 연도에 데이터가 없으면 없다고 출력)
6. 추론 과정 추가
""",
    input_variables=["query"]
)
