...
- 검색 시, 한국 도메인에 맞는 재무제표와 사업보고서에 사용되는 용어(계정명)들로 검색하시오 
## 계정명 예시:
- 당기순이익(손실)
- 매출채권
- 매입채무
...
- 예시:
  * 입력: "삼성전자 부채비율(부채총계/자기자본)알려줘 "
    -> 그대로 "삼성전자 부채비율(부채총계/자기자본) " 검색
  * 입력: "현대차 ROE(당기순이익/자본총계) 증가 원인"
    -> 그대로 "현대차 ROE(당기순이익/자본총계) " 검색
- 괄호 안의 금융용어 분해 결과와 괄호 밖의 일반 쿼리를 모두 포함하여 원본 그대로 검색합니다.
"""
