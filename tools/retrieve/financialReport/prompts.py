...
1. 사용자 요청에 명시적인 목차나 표 이름이 있는지 파악
2. 명시적인 목차나 표가 있는 경우 적절하게 해당 목차나 표 이름을 쿼리에 포함
3. 명시적인 목차나 표가 없는 경우 사용자가 요청하는 정보가 어느 목차에 있는지 파악
4. 목차의 이름을 쿼리에 맞게 작성
5. 요청이 구체적이지 않다면 연결재무제표 중에서 검색할 수 있게 쿼리를 작성

아래는 예시입니다.
질문: 영업활동현금흐름
답변: III. 재무에 관한 사항 중 연결재무제표의 연결 현금흐름표 영업활동으로 인한 현금흐름

질문: 당기순이익
답변: III. 재무에 관한 사항 중 연결재무제표 포괄손익계산서에서 당기순이익

질문: 삼성전자 매출액 구성
답변: III. 재무에 관한 사항 중 연결재무제표 주석의 매출액 구성과 영업부문별 정보

질문: LG화학 연구개발 현황
답변: II. 사업의 내용의 제조서비스업 중 주요계약 및 연구개발활동

질문: 현대차 최대주주 현황
답변: I. 회사의 개요 중 주식의 총수 등에서 최대주주 및 특수관계인의 주식소유 현황

질문: SK하이닉스 배당금
답변: III. 재무에 관한 사항 중 배당에 관한 사항

질문: 현금성자산
답변: III. 재무에 관한 사항 중 연결재무제표의 연결 현금흐름표 현금 및 현금성자산

질문: ROA
답변: II. 사업의 내용 중 재무건전성 등 기타 참고사항 ROA
...
4. 사용자의 별다른 지시가 없을 경우 연결 재무제표에서 검색 하도록 입력 쿼리를 작성해주세요
5. 사업보고서에 검색할 때는 여러 계정명을 동시에 검색하지 마세요
6. 불필요한 추가 정보 요청이나 출처는 제거해주세요
